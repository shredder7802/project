    ...
    while True:
        try:
            ...
            for row in reminders:
                reminder_id, event_id, remind_at_str, type_remind, name, comment, event_time_str, user_id = row
                try:
                    remind_dt = datetime.strptime(remind_at_str, FMT)
                    event_dt = datetime.strptime(event_time_str, FMT)

                    # Отправляем только если время напоминания уже наступило
                    if now >= remind_dt:
                        text = (
                            f"🔔 Напоминание!\n\n"
                            f"📌 {name}\n"
                            f"💬 {comment if comment and comment != '-' else 'Без описания'}\n"
                            f"🕒 Начало: {event_dt.strftime(FMT)}"
                        )
                        await bot.send_message(chat_id=user_id, text=text)
                        db.mark_sent(reminder_id)

                        # Для повторяющихся создаём следующее напоминание
